...
export async function callCommand(
  ...
  if (!wallet) {
    console.error(
      chalk.red("  ✗ No wallet found. Run: stellar-agent wallet create"),
    );
    process.exit(1);
  }
  ...
  let meta: any;
  try {
    meta = await agentRegistry.resolveAgent(nameOrAddress, network);
    logger.success("Agent found");
    logger.arrow(`Name:     ${meta.name}`);
    logger.arrow(`Endpoint: ${meta.endpoint}`);
    logger.arrow(
      `Price:    ${chalk.magenta(meta.price + " " + meta.asset)} per call`,
    );
  ...
  try {
    ...
    if (balance < price) {
      logger.error(
        `Insufficient ${meta.asset}`,
        `Have ${balance}, need ${price}`,
      );
  ...
  const newBalance =
    balancesAfter.find((b: any) => b.asset === meta.asset)?.balance || "?";
