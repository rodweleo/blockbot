...
export async function listCommand(options: { network?: string }): Promise<void> {
  logger.banner()
  ...
  try {
    agents = await listAgents(network)
  } catch (e: any) {
    logger.error("Failed to fetch registry", e.message)
    logger.info("Make sure STELLAR_REGISTRY_ADDRESS is set in your environment")
    process.exit(1)
  }
  ...
  console.log("  " + chalk.gray("─".repeat(col.name + col.price + col.model + col.desc)))

  for (const row of rows) {
    console.log(
      "  " +
      row.name.padEnd ? row.name.padEnd(col.name) : (row.name + " ".repeat(Math.max(0, col.name - row.name.length))) +
  ...
  console.log()
  console.log(chalk.gray(`  Call any agent: stellar-agent call <name> "<task>"`))
  console.log()
}
...
export async function inspectCommand(
  ...
  try {
    const meta: any = await resolveAgent(nameOrAddress, network)
    ...
    console.log(chalk.cyan(`    stellar-agent call "${meta.name}" "<your task>"`))
    console.log()
  } catch (e: any) {
